...
function OrdersScreen() {
  ...
  const handleOpenRating = (order: Order) => {
    ...
    order.orderItems.forEach((item) => {
      ...
      initialRatings[productId._id] = 0;
    });
  ...
  const handleSubmitRating = async () => {
    ...
    if (!allRated) {
      Alert.alert('Please rate all products before submitting.');
      return;
    }
    ...
    try {
      const results = await Promise.allSettled(
        selectedOrder.orderItems.map((item) => {
          const productId = item.productId;
          const rating = productRatings[productId._id];
      ...
      if (successful > 0) {
        Alert.alert(
          'Success',
          `Successfully submitted ${successful} review${successful > 1 ? 's' : ''}!`
        );
        // Refetch orders query to refresh hasReviewed status
        queryClient.refetchQueries({ queryKey: ['orders'] });
      }
      if (failed > 0) {
        Alert.alert(
          'Warning',
          `${failed} review${failed > 1 ? 's' : ''} failed to submit. Some products may have already been reviewed.`
        );
      }
      ...
      Alert.alert(
        'Error',
        error?.response?.data?.message || 'Failed to submit reviews.'
      );
    } finally {
      setShowRatingModal(false);
      setSelectedOrder(null);
